...
function StackCelebration({ data, onDone, C, dark }) {
  ...
  const bd = dark ? "1px solid rgba(247,147,26,0.4)" : `2px solid ${C.gold}`;
  return (
    <div style={{ position:"fixed", inset:0, display:"flex", alignItems:"center", justifyContent:"center", zIndex:1100, pointerEvents:"none" }}>
      <div style={{ background:bg, border:bd, borderRadius:24, padding:"28px 32px", maxWidth:360, width:"90%", textAlign:"center", boxShadow:shadowMd, opacity:visible?1:0, transform:visible?"scale(1) translateY(0)":"scale(0.85) translateY(20px)", transition:"opacity 0.4s ease, transform 0.4s cubic-bezier(0.34,1.56,0.64,1)" }}>
...
function GoalCompleteModal({ goal, onClose, C, dark }) {
  const bg = dark ? "rgba(0,0,0,0.88)" : "rgba(0,0,0,0.55)";
  const cardBg = dark ? "rgba(15,16,18,0.95)" : C.card;
  return (
    <div style={{ position:"fixed", inset:0, background:bg, backdropFilter:"blur(14px)", zIndex:1200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ background:cardBg, borderRadius:28, padding:"40px 36px", width:"100%", maxWidth:460, textAlign:"center", boxShadow:shadowMd, border:`2px solid ${C.green}` }}>
...
function MotivationBanner({ C, dark }) {
  ...
  return (
    <Card C={C} dark={dark} style={{ marginBottom:20, opacity:visible?1:0, transition:"opacity 0.5s ease", position:"relative", overflow:"hidden" }}>
  ...
  return (
    <Card C={C} dark={dark} style={{ marginBottom:16, position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ padding:"22px 24px" }}>
        <SectionLabel C={C} color={C.blue}>Global Stackers</SectionLabel>
        ...
        {mode==="preset" ? (
          ...
          <div style={{ marginBottom:24 }}>
            ...
            <input type="number" placeholder={unit==="sats"?"e.g. 5000000":"e.g. 0.05"} value={custom} onChange={e => setCustom(e.target.value)}
              onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
              style={{ ...iBase(C,dark), border:`1.5px solid ${focused?C.gold:C.border}`, boxShadow:focused?`0 0 0 3px ${C.goldLight}`:"none" }}/>
            {satsVal>0 && <div style={{ marginTop:10, padding:"10px 14px", background:C.goldLight, borderRadius:10 }}><div style={{ fontSize:10, color:dark?C.goldDim:C.goldDim, fontWeight:600 }}>{unit==="sats"?`= ₿ ${(satsVal/1e8).toFixed(8).replace(/0+$/,"").replace(/\.$/,"")}`:`= ⚡ ${satsVal.toLocaleString()} sats`}</div></div>}
          </div>
        )}
...
// ── Logo constant (injected at build time) ────────────────────────────────────
...
export default function SatTracker() {
    ...
    const unsub = onAuthChange(async (user) => {
      if (user) {
        ...
        if (data) {
          ...
          if (data.profile) { setProfile(data.profile); setAuthStep("idle"); }
  ...
  useEffect(() => {
    ...
    (async () => {
      try { await saveUserData(googleUser.uid, { _active: true }); setStackerCount(prev => Math.max(prev, 1)); } catch {}
    })();
  }, [googleUser]);
  ...
  const saveProfile = async (p) => {
    ...
    setAuthStep("idle"); setShowProfileModal(false);
  ...
  return (
    <div style={{ background:C.bg, minHeight:"100vh", color:C.text, fontFamily:"system-ui,sans-serif", transition:"background 0.3s, color 0.3s", position:"relative" }}>
      ...
      <div style={{ background:darkMode?"rgba(7,8,10,0.92)":C.navBg, borderBottom:`1px solid ${C.navBorder}`, position:"sticky", top:0, zIndex:100, backdropFilter:darkMode?"blur(16px)":"none", boxShadow:darkMode?"none":"0 1px 8px rgba(0,0,0,0.07)", transition:"background 0.3s" }}>
        <div style={{ maxWidth:920, margin:"0 auto", padding:"0 20px", height:62, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
            {/* BTC price pill */}
            <div style={{ background:darkMode?"rgba(247,147,26,0.08)":C.goldLight, border:`1px solid ${C.gold}30`, borderRadius:12, padding:"7px 14px", display:"flex", alignItems:"center", gap:10 }}>
              <div style={{ width:6, height:6, borderRadius:"50%", background:priceLoading?C.muted:C.green, boxShadow:priceLoading?"none":`0 0 6px ${C.green}` }}/>
              <div>
                <div style={{ fontSize:14, fontWeight:800, color:C.gold, letterSpacing:-0.3 }}>{btcPrice?fmtUsd(btcPrice):"———"}</div>
                <div style={{ fontSize:9, color:C.muted, lineHeight:1 }}>{priceAge?priceAge.toLocaleTimeString():"fetching…"} <span onClick={fetchPrice} style={{ color:C.gold, cursor:"pointer" }}>↻</span></div>
              </div>
            </div>
            {/* Theme toggle */}
            ...
            {profile && AvatarComp && (
              <div onClick={() => setShowProfileModal(true)}
                style={{ display:"flex", alignItems:"center", gap:9, cursor:"pointer", background:darkMode?"rgba(255,255,255,0.05)":C.cardAlt, border:`1px solid ${C.border}`, borderRadius:12, padding:"7px 12px", transition:"border-color 0.2s" }}
                onMouseEnter={e => { e.currentTarget.style.borderColor=C.gold+"50"; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor=C.border; }}>
                <div style={{ width:30, height:30, borderRadius:8, overflow:"hidden", border:`2px solid ${badge?badge.color+"60":C.gold+"40"}`, flexShrink:0 }}><AvatarComp/></div>
                <div>
                  <div style={{ fontSize:11, fontWeight:600, color:C.text }}>@{profile.username}</div>
                  {googleUser && <div style={{ fontSize:9, color:C.muted, marginTop:1 }}>{googleUser.email}</div>}
                  {badge && <div style={{ fontSize:9, color:badge.color, fontWeight:700, letterSpacing:0.5, marginTop:1 }}>{badge.emoji} {badge.label}</div>}
                </div>
              </div>
            )}
            {/* Sign out */}
      ...
      <div style={{ maxWidth:920, margin:"0 auto", padding:"24px 20px 80px", position:"relative", zIndex:1 }}>
        ...
        {/* ── Profile hero strip ── */}
        {profile && AvatarComp && (
          <Card C={C} dark={darkMode} style={{ marginBottom:16, background:darkMode?`linear-gradient(135deg,rgba(30,21,5,0.9) 0%,rgba(15,16,18,0.85) 100%)`:
            "linear-gradient(135deg,#fff8f0 0%,#fff 100%)", border:`1px solid ${C.gold}25` }}>
            <div style={{ padding:"18px 22px", display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
              <div style={{ width:52, height:52, borderRadius:14, overflow:"hidden", border:`2px solid ${badge?badge.color+"60":C.gold+"40"}`, flexShrink:0 }}><AvatarComp/></div>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:15, fontWeight:800, color:C.text }}>@{profile.username}</div>
                {googleUser && <div style={{ fontSize:10, color:C.muted, marginTop:2 }}>{googleUser.email}</div>}
                {badge
                  ? <div style={{ display:"inline-flex", alignItems:"center", gap:5, marginTop:6, background:badge.color+"18", border:`1px solid ${badge.color}40`, borderRadius:20, padding:"3px 10px" }}><span style={{ fontSize:11 }}>{badge.emoji}</span><span style={{ fontSize:9, color:badge.color, fontWeight:700, letterSpacing:1 }}>{badge.label}</span></div>
                  : <div style={{ fontSize:10, color:C.muted, marginTop:4 }}>Stack your first sat to earn a badge 🟠</div>}
              </div>
              <div style={{ textAlign:"right" }}>
                <div style={{ fontSize:10, color:C.muted, marginBottom:3 }}>stacking for</div>
                <div style={{ fontSize:22, fontWeight:900, color:C.text }}>{daysStacking}<span style={{ fontSize:11, color:C.muted, fontWeight:400, marginLeft:4 }}>days</span></div>
              </div>
            </div>
          </Card>
        )}
        ...
        <Card C={C} dark={darkMode} style={{ marginBottom:16, position:"relative", overflow:"hidden" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", borderTop:`1px solid ${C.border}`, marginTop:4 }}>
            {[
              ...
              { label:"Progress", value:`${pct.toFixed(2)}%`, sub:daysStacking>0?`${daysStacking} days stacking`:`${entries.length} entries`, color:C.green },
            ].map((s,i) => (
              <div key={i} style={{ padding:"14px 16px", borderRight:i<2?`1px solid ${C.border}`:"none", textAlign:"center" }}>
